...
//new:
void ResponseMatricesPlotting_dijet(){
	setTDRStyle();
	extraText="private work";

	// Open output file
	TFile* inputfile = (TFile*)gROOT->GetListOfFiles()->FindObject("dijet_matrices.root"); //could be handled via input? (so that plotting can be used for any root-file)
	...
	if (!inputfile || !inputfile->IsOpen()){
		...
		inputfile = TFile::Open("dijet_matrices.root", "READ");
	...
	*/
	...
	vector<Double_t> upperx = {3300, 2000, 700, 2000, 1000, 700};
	...
	for(int k=0; k!=etabins.size(); ++k){
		...
		string plotname = Form("plot_dijet_%s", etabins.at(k));
		...
		*/
		...
		//now once again with logZ
		curcanv->SetLogz();
		curcanv->SaveAs(Form("%s_log.pdf", plotname.c_str()));
