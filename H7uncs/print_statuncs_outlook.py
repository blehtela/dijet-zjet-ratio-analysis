...
#													#
#	print_statuncs.py								#
#	Created by B. Schillinger, 12.04.2020			#
#	Last modified: B.Schillinger, 12.04.2020		#
#													#
#####################################################
...
def main():
	...
	parser.add_argument('herwig_rootfile', type=TFile, nargs='?',
						help='Required argument! File with MC results that shall be plotted. (filename glob)')
	parser.add_argument('pythia_rootfile', type=TFile, nargs='?',
						help='Required argument! File with MC results that shall be plotted. (filename glob)')

	parser.add_argument('combidata_rootfile', type=TFile, nargs='?',
						help='Required argument! File with MC results that shall be plotted. (filename glob)')
	...
	herwig_rootfile = args['herwig_rootfile']
	pythia_rootfile = args['pythia_rootfile']
	combidata_rootfile = args['combidata_rootfile']
	...
	print('\n')
	print("[print_statuncs.py]: Taking data input from %s" %data_rootfile)
	print("[print_statuncs.py]: Taking herwig input from %s" %herwig_rootfile)
	print("[print_statuncs.py]: Taking pythia input from %s" %pythia_rootfile)
	print("[print_statuncs.py]: Taking combi data input from %s" %combidata_rootfile)

	#print("[print_dijet_statuncs.py]: Taking theory input from %s" %fnlo_tables)
	...
	for i in range(bin_names.size): #should usually be 6
		bname = bin_names[i]
		if(verbose==True): print "[print_statuncs.py]: Now in bin ", bname
		...
		#data
		data, data_err, data_xax, data_low_bb, data_up_bb = read_rootfile(data_rootfile, "CrossSections_byBinwidth/%s/sumhisto_%s_RunD_%s"%(bname, process, bname))
		#herwig
		herwig, herwig_err, herwig_xax, herwig_low_bb, herwig_up_bb = read_rootfile(herwig_rootfile, "CrossSections_byBinwidth/%s/sumhisto_%s_herwig7_%s"%(bname, process, bname))
		#pythia
		pythia, pythia_err, pythia_xax, pythia_low_bb, pythia_up_bb = read_rootfile(pythia_rootfile, "CrossSections_byBinwidth/%s/sumhisto_%s_pythia8_%s"%(bname, process, bname))

		#combi data RunA+RunB+RunC+RunD
		combidata, combidata_err, combidata_xax, combidata_low_bb, combidata_up_bb = read_rootfile(combidata_rootfile, "CrossSections_byBinwidth/%s/sumhisto_%s_%s"%(bname, process, bname))
		...
		if(slicing==True):
			if(process=="dijet"):
				...
				herwig_xax = herwig_xax[11:_ybysbin_xind_up[bname]]
				herwig = herwig[11:_ybysbin_xind_up[bname]]
				herwig_err = herwig_err[11:_ybysbin_xind_up[bname]]
				herwig_low_bb = herwig_low_bb[11:_ybysbin_xind_up[bname]]
				herwig_up_bb = herwig_up_bb[11:_ybysbin_xind_up[bname]]

				pythia_xax = pythia_xax[11:_ybysbin_xind_up[bname]]
				pythia  = pythia[11:_ybysbin_xind_up[bname]]
				pythia_err = pythia_err[11:_ybysbin_xind_up[bname]]
				pythia_low_bb = pythia_low_bb[11:_ybysbin_xind_up[bname]]
				pythia_up_bb = pythia_up_bb[11:_ybysbin_xind_up[bname]]

				combidata_xax = combidata_xax[11:_ybysbin_xind_up[bname]]
				combidata = combidata[11:_ybysbin_xind_up[bname]]
				combidata_err = combidata_err[11:_ybysbin_xind_up[bname]]
				combidata_low_bb = combidata_low_bb[11:_ybysbin_xind_up[bname]]
				combidata_up_bb = combidata_up_bb[11:_ybysbin_xind_up[bname]]
				...
				herwig_xax = herwig_xax[11:_ybysbin_xind_up_zjetdata[bname]]
				herwig = herwig[11:_ybysbin_xind_up_zjetdata[bname]]
				herwig_err = herwig_err[11:_ybysbin_xind_up_zjetdata[bname]]
				herwig_low_bb = herwig_low_bb[11:_ybysbin_xind_up_zjetdata[bname]]
				herwig_up_bb = herwig_up_bb[11:_ybysbin_xind_up_zjetdata[bname]]

				pythia_xax = pythia_xax[11:_ybysbin_xind_up_zjetdata[bname]]
				pythia  = pythia[11:_ybysbin_xind_up_zjetdata[bname]]
				pythia_err = pythia_err[11:_ybysbin_xind_up_zjetdata[bname]]
				pythia_low_bb = pythia_low_bb[11:_ybysbin_xind_up_zjetdata[bname]]
				pythia_up_bb = pythia_up_bb[11:_ybysbin_xind_up_zjetdata[bname]]

				combidata_xax = combidata_xax[11:_ybysbin_xind_up_zjetdata[bname]]
				combidata = combidata[11:_ybysbin_xind_up_zjetdata[bname]]
				combidata_err = combidata_err[11:_ybysbin_xind_up_zjetdata[bname]]
				combidata_low_bb = combidata_low_bb[11:_ybysbin_xind_up_zjetdata[bname]]
				combidata_up_bb = combidata_up_bb[11:_ybysbin_xind_up_zjetdata[bname]]
		...
		ratio_datadata = np.divide(data, data) #this is 1.0 always.
		ratio_herwigherwig = np.divide(herwig, herwig)
		ratio_pythiapythia = np.divide(pythia, pythia)
		data_relerr = np.divide(data_err, data)
		herwig_relerr = np.divide(herwig_err, herwig)
		pythia_relerr = np.divide(pythia_err, pythia)

		combidata_relerr=np.divide(combidata_err, combidata)

		#prepare the data rel.stat.unc.
		ones_arr = np.full(nentries, 1.0)
		data_stat_up = np.add(ones_arr, data_relerr)
		data_stat_dw = np.add(ones_arr, -1.0*data_relerr)

		combidata_up = np.add(ones_arr, combidata_relerr)
		combidata_dw = np.add(ones_arr, -1.0*combidata_relerr)
		...
		binwidths = np.subtract(data_up_bb, data_low_bb)

		nentries = data_xax.size
		#make 20% line unc arrays:
		p20_up = np.full(nentries, 1+0.20)
		p20_dw = np.full(nentries, 1-0.20)
	
		moved_xax = np.add(data_xax,0.25*binwidths) 
		#moved_xax_zjet = np.add(zjet_xax, 0.25*binwidths_zjet)
		#print "dijet_xax: ", dijet_xax
		#print "moved_xax: ", moved_xax

		plot_indiv_uncs(bname, ax_ratio_bin, patches_bin, data_xax, binbounds, p20_up, 'tomato', 'dashed', "20% rel.stat.unc")		#lumi
		plot_indiv_uncs(bname, ax_ratio_bin, patches_bin, data_xax, binbounds, p20_dw, 'tomato', 'dashed', "20% rel.stat.unc")	#lumi
		...
		runperiod="D"
		generator="generator"
		#plotting data rel. statistical uncertainty:
		plot_unc_filled(bname, ax_ratio_bin, patches_bin, data_xax, binbounds, data_stat_dw, data_stat_up, 'mediumseagreen', 'mediumseagreen', 0.6, "data Run D statistical uncertainty", runperiod, generator) #JES unc total (new name)

		#combidata
		plot_indiv_uncs(bname, ax_ratio_bin, patches_bin, combidata_xax, binbounds, combidata_up, 'royalblue', 'dotted', "Rel. stat. unc. if run periods A, B, C, D combined.")
		plot_indiv_uncs(bname, ax_ratio_bin, patches_bin, combidata_xax, binbounds, combidata_dw, 'royalblue', 'dotted', None)
		...
		ax_ratio_bin.errorbar(herwig_xax, ratio_herwigherwig, xerr=None, yerr=np.absolute(herwig_relerr), elinewidth=1.2, linewidth=1.0, marker='.', ms=2, color='darkorange', fillstyle='none', fmt='.', label="Herwig7 relative statistical uncertainty") #data stat. unc --> in greenyellow, lawngreen, darkorange
		ax_ratio_bin.errorbar(moved_xax, ratio_pythiapythia, xerr=None, yerr=np.absolute(pythia_relerr), elinewidth=1.2, linewidth=1.0, marker='.', ms=2, color='lawngreen', fillstyle='none', fmt='.', label="Pythia8 relative statistical uncertainty") #data stat. unc --> in greenyellow, lawngreen, darkorange
		...
		if(process=="dijet"):
			ax_ratio_bin.set_title("DIJET: Relative statistical uncertainties in data and MC simulations, %s"%(bname), fontsize=12)
		else: #zjet
			ax_ratio_bin.set_title("Z+JET: Relative statistical uncertainties in data and MC simulations, %s"%(bname), fontsize=12)
		...
		if(slicing==True):
			if(process=="dijet"):
				...
				ax_ratio_bin.set_xlim(50.0, _ybysbin_xlim_up_zjet[bname])
			fig_ratio_bin.savefig("%s_combidata_relStatUncs_%s_sliced.png"%(process,bname))

		else:
			fig_ratio_bin.savefig("%s_combidata_relStatUncs_%s.png"%(process,bname))
		...
		if(slicing==True):
			...
			fig_ratio_bin.savefig("%s_combidata_relStatUncs_%s_zoomedTo20_sliced.png"%(process,bname))
		else:
			fig_ratio_bin.savefig("%s_combidata_relStatUncs_%s_zoomedTo20.png"%(process,bname))


	# stop timer
	stop_time = timeit.default_timer()
	timediff = stop_time-start_time
	print("[print_statuncs.py]: Elapsed time: %s sec = %s min" %(timediff, round(timediff/60., 2)))
