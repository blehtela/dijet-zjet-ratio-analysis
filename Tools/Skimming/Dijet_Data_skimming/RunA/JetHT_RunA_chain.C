#define JetHT_RunA_chain_cxx
#include "JetHT_RunA_chain.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
// Created via MakeClass out of a TChain containing all the JetHT files for RunA
// This script is mainly used for skimming
// Skimming: Loop through events: Check if ANY of the jet-triggers fired --> if yes: keep, if no: discard

void JetHT_RunA_chain::Loop()
{
//   In a ROOT session, you can do:
//      root> .L JetHT_RunA_chain.C
//      root> JetHT_RunA_chain t
...
void JetHT_RunA_chain::Skim(){

	//create output file
	TFile *skimfile = new TFile("JetHT_RunA_skimmed.root", "recreate");
	...
	const char* dataset_title_1 = "/JetHT/Run2018A-Nano1June2019-v2/NANOAOD";
	...
	///for(int ievent=0; ievent!=100000; ++ievent){}	//for testing
		...
		//make vector containing the trigger decisions for this current event
		//what about the lower pT region? will be completely thrown out here...
		////vector<Bool_t> trg_decisions = {HLT_PFJet40, HLT_PFJet60, HLT_PFJet80, HLT_PFJet140, HLT_PFJet200, HLT_PFJet260, HLT_PFJet320, HLT_PFJet400, HLT_PFJet450, HLT_PFJet500, HLT_PFJet550};
		...
		if(nJet>1){
			...
			/*
			//check all the triggers for this dijet event --> throw event away if no trigger fired
			for(int i=0; i!=trg_decisions.size(); ++i){
				if(trg_decisions.at(i)){
					continue;
				}
			}//end triggerloop
			*/
	...
	metainfo->DrawLatex(0.06, 0.92, Form("Dataset:  #scale[1.2]{#color[30]{/JetHT/Run2018A-Nano1June2019-v2/NANOAOD}}"));
	//metainfo->DrawLatex(0.06, 0.86, Form("   "));	//empty line
