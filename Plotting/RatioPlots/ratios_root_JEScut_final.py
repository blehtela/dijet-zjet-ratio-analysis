...
# 	Last modified by B. Schillinger, 22.04.2020		   		#
#													   		#
#############################################################
...
#old version before solving JES unc issue
...
def main():
	...
	x_minticks = [50, 60, 70, 80, 90, 200, 300, 400, 500, 600, 700, 800]
	x_minticklabels = ['','60', '', '80', '', '200', '', '400', '', '600', '', '800']
	x_majticks = [100]
	x_majticklabels = ['100']
	...
	ax1_sum.set_ylim(4*1e3, 4*1e6) 	#new
	...
	if(datatype=="data"):
		...
		fig_tot.savefig("ratio_summary_data_Run%s.svg"%runperiod) #as .svg
		fig_tot.savefig("ratio_summary_data_Run%s.pdf"%runperiod) #as .pdf

	elif(datatype=="mc"):
		if(generator=="HT_madgraph_pythia8"):
			ax1_sum.set_title("Cross Section Ratios: Dijet over Z+jet\n Monte Carlo Simulation (%s)"%generatorname, fontsize=14)
			fig_tot.savefig("ratio_summary_mc_%s.png"%generator)
			fig_tot.savefig("ratio_summary_mc_%s.svg"%generator) #as .svg
			fig_tot.savefig("ratio_summary_mc_%s.pdf"%generator) #as .pdf
		else:
	...
	ax1_sum.set_xlim(50,800)

	plt.tight_layout()
	if(datatype=="data"):
		fig_tot.savefig("ratio_summary_data_Run%s_smallxrange_50to800.png"%runperiod)
		fig_tot.savefig("ratio_summary_data_Run%s_smallxrange_50to800.svg"%runperiod)	#as .svg
		fig_tot.savefig("ratio_summary_data_Run%s_smallxrange_50to800.pdf"%runperiod)	#as .pdf
	elif(datatype=="mc"):
		fig_tot.savefig("ratio_summary_mc_%s_smallxrange_50to800.png"%generator)
		fig_tot.savefig("ratio_summary_mc_%s_smallxrange_50to800.svg"%generator)	#as .svg
		fig_tot.savefig("ratio_summary_mc_%s_smallxrange_50to800.pdf"%generator)	#as .pdf
