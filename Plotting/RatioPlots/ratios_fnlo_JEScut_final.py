...
#Old version before JESunc cuts:
...
def main():
	...
	for i in range(bin_names.size): #should usually be 6
		...
		dijet_bb_LO_trans = dijet_binbounds_LO.T[:,4:_ybysbin_xind_up_zjetfnlo[bname]]	#work-around...
		dijet_binbounds_LO = dijet_bb_LO_trans.T	#getting back to old variable
		...
		print "ratio_LO_err: ", ratio_LO_err
		ratio_LO = ratio_LO[4:_ybysbin_xind_up_zjetfnlo[bname]]
		ratio_LO_err = ratio_LO_err[:,4:_ybysbin_xind_up_zjetfnlo[bname]]
	...
	ax_ratio_LO.set_xlabel(r'$\mathrm{p_{T,avg}}$ /GeV', fontsize=13, horizontalalignment='right')
	ax_ratio_LO.xaxis.set_label_coords(1.00, -0.06)
	ax_ratio_LO.set_ylabel(r'XS ratios: Dijet / Zjet', fontsize=13, horizontalalignment='center', verticalalignment='top', y=0.5, rotation=90, labelpad=24)
	...
	ax_ratio_LO.set_xticklabels(x_majticklabels, minor=False, fontsize=12)	#set major ticks label
	ax_ratio_LO.set_xticklabels(x_minticklabels, minor=True, fontsize=12)	#set minor titcks label

	#ytick labels
	for tick in ax_ratio_LO.yaxis.get_major_ticks():
		tick.label.set_fontsize(12)


	#test for suitable y-axis range
	ax_ratio_LO.set_ylim(1e3, 4*1e6)
	
	
	ax_ratio_LO.legend(fontsize=12, numpoints=1, loc='upper right')
	ax_ratio_LO.set_title("Cross Section Ratios: Dijet over Z+jet \n Fixed-Order Theory Calculations in LO (%s)"%pdfset, fontsize=14)
	plt.tight_layout()
	...
	ax_ratio_LO.set_xlim(50,1200)
	plt.tight_layout()
	fig_ratio_LO.savefig("ratio_summary_fnlo_LO_smallxrange_50to1200.png")
	...
	plt.tight_layout()
	fig_ratio_NLO.savefig("ratio_summary_fnlo_NLO_smallxrange_50to1200.png")
