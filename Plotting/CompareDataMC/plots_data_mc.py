...
#	Last modified: B.Schillinger, 03.12.2019		#
#													#
#####################################################
...
##low_bb and up_bb are used for drawing the "x-error" == binwidth
	...
	ax.errorbar(x_axis, data, xerr=0.0, yerr=data_err, elinewidth=1.2, linewidth=1.0, marker=_ybysbin_marker[bname], ms=6, color=_ybysbin_col[bname], fillstyle='none', fmt='.', label=_ybysbin_label[bname])
	ax.errorbar(x_axis, mc, xerr=0.0, yerr=mc_err, elinewidth=1.2, linewidth=1.0, marker=_ybysbin_marker[bname], ms=6, color='k', fillstyle='none', fmt='.', label=_ybysbin_label[bname])
	...
	ax.set_ylabel(r'XS Data and MC in fb', fontsize=12, horizontalalignment='center', verticalalignment='top', y=0.5, rotation=90, labelpad=24)
	...
	ax_xs_sum.errorbar(x_axis, data, xerr=0.0, yerr=data_err, elinewidth=1.2, linewidth=1.0, marker=_ybysbin_marker[bname], ms=6, color=_ybysbin_col[bname], fillstyle='none', fmt='.', label=_ybysbin_label[bname], alpha=m_alpha)
	ax_xs_sum.errorbar(x_axis, mc, xerr=0.0, yerr=mc_err, elinewidth=1.2, linewidth=1.0, marker=_ybysbin_marker[bname], ms=6, color='k', fillstyle='none', fmt='.', label=_ybysbin_label[bname], alpha=m_alpha)
...
##low_bb and up_bb are used for drawing the "x-error" == binwidth
	...
	ax.errorbar(x_axis, ratio, xerr=0.0, yerr=ratio_err, elinewidth=1.2, linewidth=1.0, marker=_ybysbin_marker[bname], ms=6, color=_ybysbin_col[bname], fillstyle='none', fmt='.', label=_ybysbin_label[bname])

	ax.grid(True, which="both", axis="y", color="k", linestyle="dotted", linewidth=1)
...
#for individual ybys bins
def plot_combi(bname, ax_xs, ax_xs_sum, ax_ratio, ax_ratio_sum, x_axis, data, data_err, mc, mc_err, ratio, ratio_err, m_alpha, process, runperiod, generator):

	#call the XS plotting function for the upper part of the plot
	plot_xs(bname, ax_xs, ax_xs_sum, x_axis, data, data_err, mc, mc_err, m_alpha, process, runperiod, generator)
	ax_xs.set_xlabel("")
	ax_xs.set_yscale("log")

	#call the ratio plotting function for the lower part of the plot
	plot_ratio(bname, ax_ratio, ax_ratio_sum, x_axis, ratio, ratio_err, m_alpha, process, runperiod, generator)
	ax_ratio.set_title("")
	ax_ratio.set_yscale("linear")
	#ax_ratio.set_ylim(-1.5, 2.0)	#test
	ax_ratio.set_ylim(0.0, 2.0)	#test
...
def main():
	...
	ax_ratio_sum = plt.subplot(xscale="log",yscale="linear")
	...
	for j in range(0, 6):
		...
		ax_xs_bin = plt.subplot(xscale="log", yscale="log")
		...
		ax_combi_xs_bin = plt.subplot(gs_combi_bin[:-1,:], xscale="log")				#upper part of plot
		ax_combi_ratio_bin = plt.subplot(gs_combi_bin[-1:,:], xscale="log")			#lower part of plot
		...
		if(process=="dijet"):
			...
			#data, data_err, data_xax, data_low_bb, data_up_bb = read_rootfile(data_rootfile, "Standard/%s/alternative_trigger_fits/sumhisto_%s"%(bname, bname))
			#mc, mc_err, mc_xax, mc_low_bb, mc_up_bb = read_rootfile(mc_rootfile, "Standard/%s/mc_histo_%s"%(bname, bname))
			...
			#data, data_err, data_xax, data_low_bb, data_up_bb = read_rootfile(data_rootfile, "Standard/%s/additional_plots_%s/sumhisto_%s"%(bname, bname, bname))	#should fix naming
			#mc, mc_err, mc_xax, mc_low_bb, mc_up_bb = read_rootfile(mc_rootfile, "Standard/%s/mc_histo_%s"%(bname, bname))
		...
		print "data_relerr: ", data_relerr, "in j=", j
		print "mc_relerr: ", mc_relerr
		...
		#ratio data/mc
		plot_ratio(bname, ax_ratio_bin, ax_ratio_sum, data_xax, cur_ratio, ratio_err, m_alpha, process, runperiod, generator)


		#combination: xs and ratio
		plot_combi(bname, ax_combi_xs_bin, ax_combi_xs, ax_combi_ratio_bin, ax_combi_ratio, data_xax, data, data_err, mc, mc_err, cur_ratio, ratio_err, m_alpha, process, runperiod, generator)
		...
		#xs plot:
		#plt.legend()
		ax_xs_bin.legend(fontsize=14, numpoints=1, loc='upper right')
		if((runperiod!=None)and(generator!=None)):
			fig_xs_bin.savefig("%s_%s_xs_data_Run%s_mc_%s.png"%(process,bname,runperiod,generator))
		else:
			fig_xs_bin.savefig("%s_%s_xs_data_mc.png"%(process,bname))

		#ratio plot:
		ax_ratio_bin.legend(fontsize=14, numpoints=1, loc='upper left')
		if((runperiod!=None)and(generator!=None)):
			fig_ratio_bin.savefig("%s_%s_ratio_data_Run%s_mc_%s.png"%(process,bname,runperiod,generator))
		else:
			fig_ratio_bin.savefig("%s_%s_ratio_data_mc.png"%(process,bname))

		#combination plot
		ax_combi_xs_bin.legend(fontsize=14, numpoints=1, loc='upper right')	#only one legend for upper plot
		if((runperiod!=None)and(generator!=None)):
			fig_combi_bin.savefig("%s_%s_combi_data_Run%s_mc_%s.png"%(process,bname,runperiod,generator))
		else:
			fig_combi_bin.savefig("%s_%s_combi_data_mc.png"%(process,bname))
	...
	#plt.xlabel(r'$\mathrm{p_{T,avg}}$ /GeV', horizontalalignment='right', loc='right')
	ax_xs_sum.set_ylabel(r'XS Data and MC in fb', fontsize=12, horizontalalignment='center', verticalalignment='top', y=0.5, rotation=90, labelpad=24)
	if((runperiod!=None)and(generator!=None)):
		ax_xs_sum.set_title(r'Cross Section Summary (%s, Run %s, %s)'%(process,runperiod,generator), fontsize=14, loc='right', horizontalalignment='right')
	else:
		ax_xs_sum.set_title(r'Cross Section Summary (%s)'%process, fontsize=14, loc='right', horizontalalignment='right')
	...
	plt.legend()
	ax_xs_sum.legend(fontsize=14, numpoints=1, loc='lower left')

	plt.tight_layout()		#especially for the combi plot

	if((generator!=None)and(runperiod!=None)):
		fig_xs.savefig("%s_xs_overview_mc_%s_data_Run%s.png"%(process,generator,runperiod))
	else:
		fig_xs.savefig("%s_xs_overview_mc_data.png"%(process))
	...
	if((runperiod!=None)and(generator!=None)):
		ax_ratio_sum.set_title(r'Data/MC Ratio Summary (%s, Run %s, %s)'%(process,runperiod,generator), fontsize=14, loc='right', horizontalalignment='right')
		fig_ratio.savefig("%s_ratio_summary_mc_%s_data_Run%s.png"%(process,generator,runperiod))
	else:
		ax_ratio_sum.set_title(r'Data/MC Ratio Summary (%s)'%process, fontsize=14, loc='right', horizontalalignment='right')
		fig_ratio.savefig("%s_ratio_summary_mc_data.png"%process)


	#COMBIPLOT SUMMARY
	ax_combi_xs.set_ylabel(r'XS Data and MC in fb', fontsize=12, horizontalalignment='center', verticalalignment='top', y=0.5, rotation=90, labelpad=24)
	ax_ratio_sum.set_yscale("linear")	#at wrong place here?
	xstart, xend = ax_combi_xs.get_xlim()
	ax_combi_ratio.set_xlim((xstart, xend))
	if((runperiod!=None)and(generator!=None)):
		ax_combi_xs.set_title(r'XS and Ratio Summary (%s, Run %s, %s)'%(process,runperiod,generator), fontsize=14, loc='right', horizontalalignment='right')
		fig_combi.savefig("%s_combi_summary_mc_%s_data_Run%s.png"%(process,generator,runperiod))
	else:
		ax_combi_xs.set_title(r'XS and Ratio Summary (%s)'%process, fontsize=14, loc='right', horizontalalignment='right')
		fig_combi.savefig("%s_combi_summary_mc_data.png"%(process))
