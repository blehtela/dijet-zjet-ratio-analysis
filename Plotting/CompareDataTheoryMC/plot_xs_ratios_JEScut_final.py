...
def read_rootfile(rootfile, objname, verbose=False):
	...
	print("[plot_xs_ratios_JEScut_final.py]: Reading %s from %s" %(objname, rootfile))
	print("--------------------------------------------------------------------------------")
...
def main():
	...
	print('\n')
	print("[plot_xs_ratios_JEScut_final.py]: Taking MC input from %s" %mc_rootfile)
	print("[plot_xs_ratios_JEScut_final.py]: Taking data input from %s" %data_rootfile)
	print("[plot_xs_ratios_JEScut_final.py]: Taking theory input from %s" %fnlo_tables)
	...
	for i in range(bin_names.size): #should usually be 6
		bname = bin_names[i]
		if(verbose==True): print "[plot_xs_ratios_JEScut_final.py]: Now in bin ", bname
		...
		if(verbose==True): print "[plot_xs_ratios_JEScut_final.py]: Reading fnlo table: ", fnlo_tables[i]
		...
		if(process=="dijet"):
			...
			scale_unc_LO = scale_unc_LO[:,4:_ybysbin_xind_up_dijetfnlo[bname]]
			scale_unc_NLO = scale_unc_NLO[:,4:_ybysbin_xind_up_dijetfnlo[bname]]
			pdf_unc_LO = pdf_unc_LO[:,4:_ybysbin_xind_up_dijetfnlo[bname]]
			pdf_unc_NLO = pdf_unc_NLO[:,4:_ybysbin_xind_up_dijetfnlo[bname]]
			...
			#NEW: additionally cut away the lower bins in order to have same lower xlim as in dijet
			...
			scale_unc_LO = scale_unc_LO[:,4:_ybysbin_xind_up_zjetfnlo[bname]]		#take everything on first axis = up and down, and entries from index 4 onwards (>56GeV)
			scale_unc_NLO = scale_unc_NLO[:,4:_ybysbin_xind_up_zjetfnlo[bname]]	
			pdf_unc_LO = pdf_unc_LO[:,4:_ybysbin_xind_up_zjetfnlo[bname]]
			pdf_unc_NLO = pdf_unc_NLO[:,4:_ybysbin_xind_up_zjetfnlo[bname]]
	

			#old version with lower pTavg values displayed:
			'''
		...
		if(process=="dijet"):
			genlabel = generatorname
		else: #zjet
			genlabel = "MadGraph+%s"%generatorname
		...
		#plot_unc_filled(bname, ax_ratio_bin, patches_bin, data_xax, binbounds, totunc_down, totunc_up, 'mediumseagreen', 'mediumseagreen', 0.6, "total JES uncertainty (H7)", runperiod, generator) #JES unc total
		...
		plot_unc_filled(bname, ax_ratio_bin, patches_bin, data_xax, binbounds, scaleunc_band_LO[0], scaleunc_band_LO[1], 'yellow', 'yellow', 0.1, "LO %s"%variation_type,runperiod,generator) #scale unc LO
		plot_unc_filled(bname, ax_ratio_bin, patches_bin, data_xax, binbounds, scaleunc_band_NLO[0], scaleunc_band_NLO[1], 'tan', 'tan', 0.6, "NLO %s"%variation_type,runperiod,generator) #scale unc NLO
		...
		ax_ratio_bin.errorbar(data_xax, ratio_datadata, xerr=None, yerr=np.absolute(ratio_data_err), elinewidth=1.2, linewidth=1.0, marker='.', ms=2, color='darkorange', fillstyle='none', fmt='.', label="Data (2018 Run %s) stat. unc."%runperiod) #data stat. unc --> in greenyellow, lawngreen, darkorange
		...
		plot_ratio(bname, ax_ratio_bin, data_xax, ratio_fnloLOdata, np.absolute(ratio_fnloLO_pdfunc), binbounds[0,:], binbounds[1,:], 'gray', m_alpha, "LO theory (%s) with PDF unc."%(pdfset)) #LO/data
		plot_ratio(bname, ax_ratio_bin, data_xax, ratio_fnloNLOdata, np.absolute(ratio_fnloNLO_pdfunc), binbounds[0,:], binbounds[1,:], 'black', m_alpha, "NLO theory (%s) with PDF unc."%(pdfset)) #NLO/data
		...
		ax_ratio_bin.set_ylabel(r'Ratio to data (2018 Run D)', fontsize=13, horizontalalignment='center', verticalalignment='top', y=0.5, rotation=90, labelpad=24)
		...
		if(process=="dijet"):
			ax_ratio_bin.set_title("Dijet XS comparison of simulation to data in %s (%s)"%(bname, _ybysbin_label[bname]), fontsize=12)

		else: #zjet
			ax_ratio_bin.set_title("Z+jet XS comparison of simulation to data in %s (%s)"%(bname, _ybysbin_label[bname]), fontsize=12)
		...
		#unc_legend = plt.legend([ratiohandles[1],ratiohandles[2],ratiohandles[0],("royalblue","dashed"),("tomato","dashed"),("khaki","dashed")], [ratiolabels[1],ratiolabels[2],ratiolabels[0],"jesTotalNoFlavor unc. up, down (H7)",'%s unc. up, down (H7)'%flavname, r"Luminosity uncertainty ($\pm$ 2.5%)"], handler_map={tuple: AnyObjectHandler()}, loc='lower left')	#make the unc legend
		...
		plt.tight_layout()
		#fig_xs.savefig("%s_xs_ratios_JEScut_final_theory_Run%s_%s.png"%(process, runperiod, generator))
		...
		#save also as .svg and .pdf
		fig_ratio_bin.savefig("%s_xs_ratios_%s_Run%s_%s_%s.svg"%(process, bname, runperiod, generator, pdfset))	#svg
		fig_ratio_bin.savefig("%s_xs_ratios_%s_Run%s_%s_%s.pdf"%(process, bname, runperiod, generator, pdfset)) #pdf


		## NEW NEW NEW
		if((process=="zjet")and(bname=='yb0ys2')):
			...
			#get bbox
			ratio_bbox = ratio_legend.get_bbox_to_anchor().inverse_transformed(ax_ratio_bin.transAxes)
			unc_bbox = unc_legend.get_bbox_to_anchor().inverse_transformed(ax_ratio_bin.transAxes)

			#change location
			##ratio_bbox.y0 -= 0.86
			##ratio_bbox.y1 -= 0.86
			ratio_legend.set_bbox_to_anchor(ratio_bbox, transform=ax_ratio_bin.transAxes)
			#ratio_legend.set_bbox_to_anchor('upper left', transform=ax_ratio_bin.transAxes)
			...
			unc_bbox.y0 += 0.73
			unc_bbox.y1 += 0.73
			unc_legend.set_bbox_to_anchor(unc_bbox, transform=ax_ratio_bin.transAxes)
			##unc_legend.set_bbox_to_anchor('upper right', transform=ax_ratio_bin.transAxes)



			#print (ratio_legend.loc='upper right')
			#ax_doubleratio_bin.set_ylim(0.40, 3.0) 	#set bigger y-range for extreme bins
			#fig_doubleratio_bin.savefig("doubleratios_%s_Run%s_%s_%s_bigger-y-range_0.4to3.0_customleg.png"%(bname, runperiod, generator, pdfset))
			#ax_doubleratio_bin.set_ylim(0.30, 2.90) 	#set bigger y-range for extreme bins
			#fig_doubleratio_bin.savefig("doubleratios_%s_Run%s_%s_%s_bigger-y-range_0.3to2.9_customleg.png"%(bname, runperiod, generator, pdfset))


			fig_ratio_bin.savefig("%s_xs_ratios_%s_Run%s_%s_%s_newlegpos_test.png"%(process, bname, runperiod, generator, pdfset))	#png
			fig_ratio_bin.savefig("%s_xs_ratios_%s_Run%s_%s_%s_newlegpos_test.svg"%(process, bname, runperiod, generator, pdfset))	#svg
			fig_ratio_bin.savefig("%s_xs_ratios_%s_Run%s_%s_%s_newlegpos_test.pdf"%(process, bname, runperiod, generator, pdfset))	#pdf
	...
	print("[plot_xs_ratios_JEScut_final.py]: Elapsed time: %s sec = %s min" %(timediff, round(timediff/60., 2)))
