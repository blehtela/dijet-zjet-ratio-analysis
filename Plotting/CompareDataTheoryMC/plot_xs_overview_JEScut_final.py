...
#														#
#	XS overview plots :									#
#	plot_xs_overview_JEScut_final.py					#
#														#
...
#														#
#	> MC and theory only displayed as lines.			#
#	> XS are multiplied by factor 10^x					#
#	> the extreme ybys bins have to be correctly		#
#		normalised --> factor (1/0.4) for yb2ys0 and 	#
#		yb0ys2 binwidth.								#
#	New in comparison to plot_xs_overview.py :			#
#	> removed "Table" and "in each.." in legend			#
#	> removed "Data RunD" from legend					#
#	> start x-axis at 50 GeV							#
#	> adjust the 10^x factors avoiding overlap			#
#														#
#	The original version of the script					#
#	can be found here: 									#
#	plot_xs_overview_old_markeres.py					#
#														#
...
#old version of the cuts, before taking JES unc issue into account:
...
#factor to multiply XS width (for nicer display), the two extreme bins additionally need the corrective factor (1/0.4) due to their binwidth!=1.0
#(1/0.4) = 1/(2/5) = 5/2 = 2.5
#_ybysbin_factor = {'yb0ys0':pow(10,4), 'yb0ys1':pow(10,2), 'yb0ys2':2.5*pow(10,0), 'yb1ys0':pow(10,3), 'yb1ys1':pow(10,2), 'yb2ys0':2.5*pow(10,1)}	
#_ybysbin_faclab = {'yb0ys0':r'$\mathrm{\times 10^4}$', 'yb0ys1':r'$\mathrm{\times 10^2}$', 'yb0ys2':r'$\mathrm{\times 10^0}$', 'yb1ys0':r'$\mathrm{\times 10^3}$', 'yb1ys1':r'$\mathrm{\times 10^2}$', 'yb2ys0':r'$\times 10^1$'}	

# adjusted version for final plots:
#_ybysbin_factor = {'yb0ys0':pow(10,6), 'yb0ys1':pow(10,3), 'yb0ys2':2.5*pow(10,0), 'yb1ys0':pow(10,5), 'yb1ys1':pow(10,3), 'yb2ys0':2.5*pow(10,2)}	
#_ybysbin_faclab = {'yb0ys0':r'$\mathrm{\times 10^6}$', 'yb0ys1':r'$\mathrm{\times 10^3}$', 'yb0ys2':r'$\mathrm{\times 10^0}$', 'yb1ys0':r'$\mathrm{\times 10^5}$', 'yb1ys1':r'$\mathrm{\times 10^3}$', 'yb2ys0':r'$\times 10^2$'}	

#do it differently for Z+jet (yb2ys0 and yb0ys1 are different from dijet)
#_ybysbin_factor_zjet = {'yb0ys0':pow(10,6), 'yb0ys1':pow(10,4), 'yb0ys2':2.5*pow(10,0), 'yb1ys0':pow(10,5), 'yb1ys1':pow(10,3), 'yb2ys0':2.5*pow(10,1)}	
#_ybysbin_faclab_zjet = {'yb0ys0':r'$\mathrm{\times 10^6}$', 'yb0ys1':r'$\mathrm{\times 10^4}$', 'yb0ys2':r'$\mathrm{\times 10^0}$', 'yb1ys0':r'$\mathrm{\times 10^5}$', 'yb1ys1':r'$\mathrm{\times 10^3}$', 'yb2ys0':r'$\times 10^1$'}	


## TEST ## --> NEW!!
_ybysbin_factor = {'yb0ys0':pow(10,6), 'yb0ys1':pow(10,3), 'yb0ys2':2.5*pow(10,0), 'yb1ys0':pow(10,5), 'yb1ys1':pow(10,3), 'yb2ys0':2.5*pow(10,1)}	#TEST
_ybysbin_faclab = {'yb0ys0':r'$\mathrm{\times 10^6}$', 'yb0ys1':r'$\mathrm{\times 10^3}$', 'yb0ys2':r'$\mathrm{\times 10^0}$', 'yb1ys0':r'$\mathrm{\times 10^5}$', 'yb1ys1':r'$\mathrm{\times 10^3}$', 'yb2ys0':r'$\times 10^1$'}	#TEST

#do it differently for Z+jet (yb2ys0 and yb0ys1 are different from dijet) ## --> NEW!!
_ybysbin_factor_zjet = {'yb0ys0':pow(10,4), 'yb0ys1':pow(10,2), 'yb0ys2':2.5*pow(10,0), 'yb1ys0':pow(10,3), 'yb1ys1':pow(10,2), 'yb2ys0':2.5*pow(10,0)}	
_ybysbin_faclab_zjet = {'yb0ys0':r'$\mathrm{\times 10^4}$', 'yb0ys1':r'$\mathrm{\times 10^2}$', 'yb0ys2':r'$\mathrm{\times 10^0}$', 'yb1ys0':r'$\mathrm{\times 10^3}$', 'yb1ys1':r'$\mathrm{\times 10^2}$', 'yb2ys0':r'$\times 10^0$'}	
...
##low_bb and up_bb are used for drawing the "x-error" == binwidth
	...
	#ax.set_ylabel(r'XS $\left(\mathbf{\frac{d\sigma}{dp_{T,avg}\/dy_b\/dy^{\ast}}}\right)$ in Data, Theory and MC in fb/GeV', fontsize=12, horizontalalignment='center', verticalalignment='top', y=0.5, rotation=90, labelpad=24)
	#ax.set_ylabel(r'Cross Section $\left(\mathbf{\frac{d\sigma}{dp_{T,avg}\/dy_b\/dy^{\ast}}}\right)$ in Data, Theory and MC in fb/GeV', fontsize=12, horizontalalignment='center', verticalalignment='top', y=0.5, rotation=90, labelpad=24)
	
	ax.set_ylabel(r'$\left(\mathbf{\frac{d^3\sigma}{dp_{T,avg}\/dy_b\/dy^{\ast}}}\right)$ in fb/GeV', fontsize=13, horizontalalignment='center', verticalalignment='top', y=0.5, rotation=90, labelpad=24)
	#ax.set_ylabel(r'$\left(\mathbf{\frac{d^3\sigma}{dp_{T,avg}\/dy_b\/dy^{\ast}}}\right) \bigg/ \left(\text{fb/GeV}\right)$', fontsize=12, horizontalalignment='center', verticalalignment='top', y=0.5, rotation=90, labelpad=24)
...
def main():
	...
	if(generator=="pythia8"):
		...
		generatorname ="MadGraph+Pythia8"
	...
	for i in range(bin_names.size): #should usually be 6
		...
		if(process=="zjet"):
			#originally displaying all available data points in zjet
			'''
			...
			fnloLO_err = fnloLO_err[:_ybysbin_xind_up_zjetfnlo[bname]]
			fnloNLO_err = fnloNLO_err[:_ybysbin_xind_up_zjetfnlo[bname]]
			'''

			#-----------------------------------------------------------
			#final version: same "low pT cut as in dijet" --> remove some bins although they are filled
			...
			#now the fnlo stuff with different binning --> cut away the higher bins that are no present in data
			fnloxs_LO = fnloxs_LO[4:_ybysbin_xind_up_zjetfnlo[bname]]
			fnloxs_NLO = fnloxs_NLO[4:_ybysbin_xind_up_zjetfnlo[bname]]
			fnloLO_err = fnloLO_err[:,4:_ybysbin_xind_up_zjetfnlo[bname]]
			fnloNLO_err = fnloNLO_err[:,4:_ybysbin_xind_up_zjetfnlo[bname]]
			...
			#now also fnlo needs adjustment to restrictions from JES unc studies
			fnloxs_LO = fnloxs_LO[4:_ybysbin_xind_up_dijetfnlo[bname]]
			fnloxs_NLO = fnloxs_NLO[4:_ybysbin_xind_up_dijetfnlo[bname]]
			fnloLO_err = fnloLO_err[:,4:_ybysbin_xind_up_dijetfnlo[bname]]
			fnloNLO_err = fnloNLO_err[:,4:_ybysbin_xind_up_dijetfnlo[bname]]
		...
		#-----------------------------------------------#
		#final version --> removed older attempts --> for those see previous script: plot_xs_overview.py
		if(verbose==True):
			print "binbounds[1]", binbounds[1]
			print "binbounds[0]", binbounds[0]
			print "binbounds.T.flatten()", binbounds.T.flatten()
			print "steppify_bin(mc)", steppify_bin(mc)
			print "steppify_bin(fnloxs_LO)", steppify_bin(fnloxs_LO)
			print "steppify_bin(fnloxs_NLO)", steppify_bin(fnloxs_NLO)
			print "data: ", data

		if(process=="zjet"):
			#with steppify: MC and FixedOrder theory
			ax_xs.step(binbounds.T.flatten(), _ybysbin_factor_zjet[bname]*steppify_bin(mc), color='saddlebrown', linestyle='solid', linewidth=1.2, where='pre', label=None, alpha=0.9)#mc
			ax_xs.step(binbounds.T.flatten(), _ybysbin_factor_zjet[bname]*steppify_bin(fnloxs_LO), color='gray', linestyle='solid', linewidth=1.2, where='pre', label=None, alpha=0.9)#fnlo LO
			ax_xs.step(binbounds.T.flatten(), _ybysbin_factor_zjet[bname]*steppify_bin(fnloxs_NLO), color='black', linestyle='solid', linewidth=1.2, where='pre', label=None, alpha=0.9)#fnlo NLO

			#data final version:
			plot_xs(bname,ax_xs, data_xax, _ybysbin_factor_zjet[bname]*data, _ybysbin_factor_zjet[bname]*data_err, _ybysbin_col[bname], 7, m_alpha, process, "%s  (%s)"%(_ybysbin_label[bname], _ybysbin_faclab_zjet[bname]), True) #data -- marker size originally 6

			# for final version: added year (2018) in title --> Z+jet: madgraph added, removed word "with" before pdf
			#ax_xs.set_title("%s XS overview (Data 2018 Run%s, MC madgraph+%s, FixedOrder Theory with %s)"%(process, runperiod, generator, pdfset), fontsize=11)
			#ax_xs.set_title("Z+jet XS overview (Data 2018 Run%s, MC madgraph+%s, FixedOrder Theory %s)"%(runperiod, generator, pdfset), fontsize=11)
			ax_xs.set_title("Z+jet Cross Section Overview", fontsize=13)


		else: #dijet
			#test with steppify
			ax_xs.step(binbounds.T.flatten(), _ybysbin_factor[bname]*steppify_bin(mc), color='saddlebrown', linestyle='solid', linewidth=1.2, where='pre', label=None, alpha=0.9)#mc
			ax_xs.step(binbounds.T.flatten(), _ybysbin_factor[bname]*steppify_bin(fnloxs_LO), color='gray', linestyle='solid', linewidth=1.2, where='pre', label=None, alpha=0.9)#fnlo LO
			ax_xs.step(binbounds.T.flatten(), _ybysbin_factor[bname]*steppify_bin(fnloxs_NLO), color='black', linestyle='solid', linewidth=1.2, where='pre', label=None, alpha=0.9)#fnlo NLO

			#data final version:
			plot_xs(bname,ax_xs, data_xax, _ybysbin_factor[bname]*data, _ybysbin_factor[bname]*data_err, _ybysbin_col[bname], 7, m_alpha, process, "%s  (%s)"%(_ybysbin_label[bname], _ybysbin_faclab[bname]), True) #data -- marker size originally 6

			# for final version: added year (2018) in title, also wrote process out 
			#ax_xs.set_title("Dijet XS overview (Data 2018 Run%s, MC %s, FixedOrder Theory %s)"%(runperiod, generator, pdfset), fontsize=11)
			ax_xs.set_title("Dijet Cross Section Overview", fontsize=13)
	...
	# legend and save 	#
	#-------------------#
	#decide about place for main-legend
	if(process=="dijet"):
		#mainloc = 'lower left'
		#theoloc = 'upper right'
		mainloc = 'upper right'
		theoloc = 'lower left'
		legend_elements = [Line2D([0],[0],color='saddlebrown', lw=4, label="MC simulation (%s)"%(generatorname)), Line2D([0],[0],color='gray', lw=4, label="LO fixed-order theory (%s)"%(pdfset)), Line2D([0],[0],color='black', lw=4, label="NLO fixed-order theory (%s)"%(pdfset))] #moved here, due to generator labeling

	else: #zjet
		#mainloc = 'upper left'
		mainloc = 'upper right'
		theoloc = 'upper left'
		legend_elements = [Line2D([0],[0],color='saddlebrown', lw=4, label="MC simulation (MadGraph+%s)"%(generatorname)), Line2D([0],[0],color='gray', lw=4, label="LO fixed-order theory (%s)"%(pdfset)), Line2D([0],[0],color='black', lw=4, label="NLO fixed-order theory (%s)"%(pdfset))] #moved here, because of madgraph label
	...
	# think about hard-coding the generator name here in the legend (dijet: pythia8, zjet: madgraph+pythia8)
	#legend_elements = [Line2D([0],[0],color='saddlebrown', lw=4, label="MC %s"%(generator)), Line2D([0],[0],color='gray', lw=4, label="LO %s"%(pdfset)), Line2D([0],[0],color='black', lw=4, label="NLO %s"%(pdfset))]
	...
	#ax_xs.legend(fontsize=10, title="Data 2018 Run D", numpoints=1, loc=mainloc)	#legend location depending on whether zjet or dijet, NEW: title
	#now in legend title: lumi
	ax_xs.legend(fontsize=10, title="Data 2018 Run D (31.93 fb$^{-1}$)", numpoints=1, loc=mainloc)	#legend location depending on whether zjet or dijet, NEW: title
	first_legend = ax_xs.get_legend()
	ax_xs.add_artist(first_legend)
	second_legend = plt.legend(handles=legend_elements, loc=theoloc) #removed legend-title again
	#second_legend = plt.legend(handles=legend_elements, title="Simulation: MC and fixed-order theory", loc=theoloc) #here as well --> legend loc depending on dijet or zjet, title:"Simulation: MC and fixed-order theory"

	firleg_title = first_legend.get_title()
	firleg_title.set_fontsize(11)
	secleg_title = second_legend.get_title()
	secleg_title.set_fontsize(11)

	#align legend entries to the left
	'''
	for tex in second_legend.get_texts():
		print tex
		tex.set_ha('left')
	'''
	...
	#now: make difference between dijet and zjet --> zjet does not extend to such high pT, remove higher bins
	if(process=="zjet"):
		# start at 50 GeV:
		#x_minticks = [50, 60, 70, 80, 90, 200, 300, 400, 500, 600, 700, 800, 900]
		#x_minticklabels = ['','60', '', '', '', '200', '300', '', '500','', '700', '', ''] #added 300
		#x_majticks = [100, 1000]
		#x_majticklabels = ['100', '1000']

		x_minticks = [50, 60, 70, 80, 90, 200, 300, 400, 500, 600, 700, 800]	#after JES unc adjustments
		x_minticklabels = ['','60', '', '80', '', '200', '300', '400', '','600', '', '800'] #added some 
		x_majticks = [100]
		x_majticklabels = ['100']



		#y_minticks = [1e-3, 1e-1, 1e1, 1e3, 1e5, 1e7, 1e9, 1e11]
		#y_minticklabels = ['$10^{-3}$', '$10^{-1}$', '$10^1$', '$10^3$', '$10^5$', '$10^7$', '$10^9$', '$10^{11}$']
		#y_majticks = [1e-2, 1e0, 1e2, 1e4, 1e6, 1e8, 1e10, 1e12]
		#y_majticklabels = ['$10^{-2}$', '$10^{0}$', '$10^2$', '$10^4$', '$10^6$', '$10^8$', '$10^{10}$', '$10^{12}$']

		y_minticks = [1e-1, 1e1, 1e3, 1e5, 1e7, 1e9]
		y_minticklabels = ['$10^{-1}$', '$10^1$', '$10^3$', '$10^5$', '$10^7$', '$10^9$']
		y_majticks = [1e-2, 1e0, 1e2, 1e4, 1e6, 1e8]
		y_majticklabels = ['$10^{-2}$', '$10^{0}$', '$10^2$', '$10^4$', '$10^6$', '$10^8$']



		#set axis limits
		#ax_xs.set_xlim(50,1100)
		#ax_xs.set_ylim(1e-3,1e12)
		ax_xs.set_xlim(50,800)
		ax_xs.set_ylim(1e-1,1e9)	#new after JES unc cuts (H7)
		...
		#y_minticks = [1e-3, 1e-1, 1e1, 1e3, 1e5, 1e7, 1e9, 1e11, 1e13, 1e15]
		#y_minticklabels = ['$10^{-3}$', '$10^{-1}$', '$10^1$', '$10^3$', '$10^5$', '$10^7$', '$10^9$', '$10^{11}$', '$10^{13}$', '$10^{15}$']
		#y_majticks = [1e-2, 1e0, 1e2, 1e4, 1e6, 1e8, 1e10, 1e12, 1e14]
		#y_majticklabels = ['$10^{-2}$', '$10^{0}$', '$10^2$', '$10^4$', '$10^6$', '$10^8$', '$10^{10}$', '$10^{12}$', '$10^{14}$']

		y_minticks = [1e-1, 1e1, 1e3, 1e5, 1e7, 1e9, 1e11, 1e13, 1e15]	#after new JES unc cuts
		y_minticklabels = ['$10^{-1}$', '$10^1$', '$10^3$', '$10^5$', '$10^7$', '$10^9$', '$10^{11}$', '$10^{13}$', '$10^{15}$'] #after new JES unc cuts
		y_majticks = [1e0, 1e2, 1e4, 1e6, 1e8, 1e10, 1e12, 1e14]
		y_majticklabels = ['$10^{0}$', '$10^2$', '$10^4$', '$10^6$', '$10^8$', '$10^{10}$', '$10^{12}$', '$10^{14}$']

		#set axis limits
		ax_xs.set_xlim(50,4000)
		#ax_xs.set_ylim(1e-3,1e15)
		ax_xs.set_ylim(1e-1,1e15)	#new after JES unc cuts (H7)
	...
	# range has to be increased, as adjusting the factors 10^x
	#y_minticks = [1e-3, 1e-1, 1e1, 1e3, 1e5, 1e7, 1e9, 1e11]
	#y_minticklabels = ['$10^{-3}$', '$10^{-1}$', '$10^1$', '$10^3$', '$10^5$', '$10^7$', '$10^9$', '$10^{11}$']
	#y_majticks = [1e-2, 1e0, 1e2, 1e4, 1e6, 1e8, 1e10, 1e12]
	#y_majticklabels = ['$10^{-2}$', '$10^{0}$', '$10^2$', '$10^4$', '$10^6$', '$10^8$', '$10^{10}$', '$10^{12}$']

	# range had to be increased, as adjusting the factors 10^x
	#y_minticks = [1e-3, 1e-1, 1e1, 1e3, 1e5, 1e7, 1e9, 1e11, 1e13]
	#y_minticklabels = ['$10^{-3}$', '$10^{-1}$', '$10^1$', '$10^3$', '$10^5$', '$10^7$', '$10^9$', '$10^{11}$', '$10^{13}$']
	#y_majticks = [1e-2, 1e0, 1e2, 1e4, 1e6, 1e8, 1e10, 1e12, 1e14]
	#y_majticklabels = ['$10^{-2}$', '$10^{0}$', '$10^2$', '$10^4$', '$10^6$', '$10^8$', '$10^{10}$', '$10^{12}$', '$10^{14}$']

	ax_xs.set_yticks(y_majticks, minor=False)            #set major ticks loc
	ax_xs.set_yticks(y_minticks, minor=True)             #set minor ticks loc
	ax_xs.set_yticklabels(y_majticklabels, minor=False)  #set major ticks label
	ax_xs.set_yticklabels(y_minticklabels, minor=True)   #set minor titcks label
	...
	if(process=="dijet"):
		...
		y_minticks = [1e-7, 1e-5, 1e-3, 1e-1, 1e1, 1e3, 1e5, 1e7, 1e9]
		y_minticklabels = ['$10^{-7}$', '$10^{-5}$', '$10^{-3}$', '$10^{-1}$', '$10^1$', '$10^3$', '$10^5$', '$10^7$', '$10^9$']
		y_majticks = [1e-6, 1e-4, 1e-2, 1e0, 1e2, 1e4, 1e6, 1e8, 1e10]
		y_majticklabels = ['$10^{-6}$', '$10^{-4}$', '$10^{-2}$', '$10^{0}$', '$10^2$', '$10^4$', '$10^6$', '$10^8$', '$10^{10}$', '$10^{12}$']
		ax_xs.set_yticks(y_majticks, minor=False)            #set major ticks loc
		ax_xs.set_yticks(y_minticks, minor=True)             #set minor ticks loc
		ax_xs.set_yticklabels(y_majticklabels, minor=False)  #set major ticks label
		ax_xs.set_yticklabels(y_minticklabels, minor=True)   #set minor titcks label
	...
	for tick in ax_xs.yaxis.get_minor_ticks():
		tick.label.set_fontsize(8)

	ax_xs.grid(True, which="major", axis="y", color="k", linestyle="dotted", linewidth=1)
	ax_xs.grid(True, which="minor", axis="y", color="k", linestyle="dotted", linewidth=1, alpha=0.8)

	plt.tight_layout()
	fig_xs.savefig("%s_xs_overview_theory_Run%s_%s.png"%(process, runperiod, generator))

	#save additionally as svg
	fig_xs.savefig("%s_xs_overview_theory_Run%s_%s.svg"%(process, runperiod, generator))

	#save additionally as pdf
	fig_xs.savefig("%s_xs_overview_theory_Run%s_%s.pdf"%(process, runperiod, generator))
