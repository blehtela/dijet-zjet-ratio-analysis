...
def main():
	...
	#	plot_xs(bname, ax_xs, data_xax, mc, mc_err, 'saddlebrown', 4, m_alpha, process, "MC %s in %s"%(generator, bname), False)#mc
		...
		#just the lines for MC and theory
		#ax_xs.step(binbounds[1], _ybysbin_factor[bname]*mc, color='saddlebrown', linestyle='solid', linewidth=1, where='mid', label=None, alpha=0.9)#mc
		#ax_xs.step(binbounds[1], _ybysbin_factor[bname]*fnloxs_LO, color='gray', linestyle='solid', linewidth=1, where='post', label=None, alpha=0.9)#fnlo LO
		#ax_xs.step(binbounds[1], _ybysbin_factor[bname]*fnloxs_NLO, color='black', linestyle='solid', linewidth=1, where='pre', label=None, alpha=0.9)#fnlo NLO
		...
		#with factors applied:	 -- should be removed, just now for testing whether "step" above works correctly
		##plot_xs(bname, ax_xs, data_xax, _ybysbin_factor[bname]*mc, _ybysbin_factor[bname]*mc_err, 'saddlebrown', 4, m_alpha, process, "MC %s in %s"%(generator, bname), False)#mc
		##plot_xs(bname, ax_xs, data_xax, _ybysbin_factor[bname]*fnloxs_LO, _ybysbin_factor[bname]*fnloLO_err, 'gray', 4, m_alpha, process, "LO Table %s in %s"%(pdfset, bname), False)#fnlo LO
		##plot_xs(bname, ax_xs, data_xax, _ybysbin_factor[bname]*fnloxs_NLO, _ybysbin_factor[bname]*fnloNLO_err, 'black', 4, m_alpha, process, "NLO Table %s in %s"%(pdfset, bname), False)#fnlo LO
		plot_xs(bname,ax_xs, data_xax, _ybysbin_factor[bname]*data, _ybysbin_factor[bname]*data_err, _ybysbin_col[bname], 7, m_alpha, process, "Data Run%s in %s (%s)"%(runperiod, _ybysbin_label[bname], _ybysbin_faclab[bname]), True) #data -- marker size originally 6
	...
	ax_xs.legend(fontsize=10, numpoints=1, loc=mainloc)	#legend location depending on whether zjet or dijet
