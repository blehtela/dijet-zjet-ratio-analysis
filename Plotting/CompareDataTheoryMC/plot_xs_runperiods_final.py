...
#	Last modified: B.Schillinger, 21.04.2020		#
#													#
#####################################################
...
def main():
	...
	for i in range(6):	#loops from 0 to 5
		...
		#--- NEW VERSION: just SCALE by data set D
		ratioA_err = np.divide(dataA_err, dataD)
		ratioB_err = np.divide(dataB_err, dataD)
		ratioC_err = np.divide(dataC_err, dataD)
		ratioD_err = np.divide(dataD_err, dataD)
		...
		'''
		plot_ratio(bname, ax_ratio, dataA_xax, ratio_AtoD, dataA_relerr, 'gold', m_alpha, process, "RunA/RunD")			#A over D
		plot_ratio(bname, ax_ratio, dataA_xax, ratio_BtoD, dataB_relerr, 'orangered', m_alpha, process, "RunB/RunD")		#B over D
		plot_ratio(bname, ax_ratio, dataA_xax, ratio_CtoD, dataC_relerr, 'darkturquoise', m_alpha, process, "RunC/RunD")	#C over D
		plot_ratio(bname, ax_ratio, dataA_xax, ratio_DtoD, dataD_relerr, 'yellowgreen', m_alpha, process, "RunD/RunD")	#D over D
		'''	
		...
		ax_ratio.set_xticks(x_majticks, minor=False)	#set major ticks loc
		ax_ratio.set_xticks(x_minticks, minor=True)		#set minor ticks loc
		ax_ratio.set_xticklabels(x_majticklabels, minor=False, fontsize=12)	#set major ticks label
		ax_ratio.set_xticklabels(x_minticklabels, minor=True, fontsize=12)	#set minor ticks label
		...
		plt.tight_layout()
		fig_ratio.savefig("%s_xs_runperiods_ratios_%s.png"%(process, bname))	#png
		fig_ratio.savefig("%s_xs_runperiods_ratios_%s.svg"%(process, bname))	#svg
		fig_ratio.savefig("%s_xs_runperiods_ratios_%s.pdf"%(process, bname))	#pdf
