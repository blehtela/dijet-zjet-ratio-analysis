...
#	Last modified: B.Schillinger, 20.04.2020		#
#													#
#####################################################
...
def main():
	...
	for i in range(bin_names.size): #should usually be 6
		...
		#make 14% line unc arrays:
		p14_up = np.full(nentries, 1+0.14)
		p14_dw = np.full(nentries, 1-0.14)
		...
		plot_indiv_uncs(bname, ax_ratio_bin, patches_bin, data_xax, binbounds, p14_up, 'tomato', 'dashed', "14% rel.stat.unc")		#lumi
		plot_indiv_uncs(bname, ax_ratio_bin, patches_bin, data_xax, binbounds, p14_dw, 'tomato', 'dashed', "14% rel.stat.unc")	#lumi
		...
		plot_unc_filled(bname, ax_ratio_bin, patches_bin, data_xax, binbounds, data_stat_dw, data_stat_up, 'mediumseagreen', 'mediumseagreen', 0.6, "Data 2018 Run D rel. stat. uncertainty", runperiod, generator) #JES unc total (new name)
		...
		ax_ratio_bin.errorbar(herwig_xax, ratio_herwigherwig, xerr=None, yerr=np.absolute(herwig_relerr), elinewidth=2.0, linewidth=1.0, marker='.', ms=2, color='darkorange', fillstyle='none', fmt='.', label="Herwig7 relative statistical uncertainty") #data stat. unc --> in greenyellow, lawngreen, darkorange
		...
		ax_ratio_bin.errorbar(moved_xax, ratio_pythiapythia, xerr=None, yerr=np.absolute(pythia_relerr), elinewidth=1.2, linewidth=1.0, marker='.', ms=2, color='royalblue', fillstyle='none', fmt='.', label="Pythia8 relative statistical uncertainty") #data stat. unc --> in greenyellow, lawngreen, darkorange
		...
		if(slicing==True):
			if(process=="dijet"):
				...
				ax_ratio_bin.set_xlim(50.0, _ybysbin_xlim_up_zjet[bname])
			fig_ratio_bin.savefig("%s_rel_stat_uncs_%s_sliced_blue.png"%(process,bname))
			fig_ratio_bin.savefig("%s_rel_stat_uncs_%s_sliced_blue.svg"%(process,bname))	#svg
			fig_ratio_bin.savefig("%s_rel_stat_uncs_%s_sliced_blue.pdf"%(process,bname))	#pdf

		else:
			fig_ratio_bin.savefig("%s_rel_stat_uncs_%s_blue.png"%(process,bname))
			fig_ratio_bin.savefig("%s_rel_stat_uncs_%s_blue.svg"%(process,bname))	#svg
			fig_ratio_bin.savefig("%s_rel_stat_uncs_%s_blue.pdf"%(process,bname))	#pdf
		...
		if(slicing==True):
			...
			fig_ratio_bin.savefig("%s_rel_stat_uncs_%s_zoomedTo20_sliced_blue.png"%(process,bname))
			fig_ratio_bin.savefig("%s_rel_stat_uncs_%s_zoomedTo20_sliced_blue.svg"%(process,bname))	#svg
			fig_ratio_bin.savefig("%s_rel_stat_uncs_%s_zoomedTo20_sliced_blue.pdf"%(process,bname))	#pdf

		else:
			fig_ratio_bin.savefig("%s_rel_stat_uncs_%s_zoomedTo20_blue.png"%(process,bname))
			fig_ratio_bin.savefig("%s_rel_stat_uncs_%s_zoomedTo20_blue.svg"%(process,bname))	#svg
			fig_ratio_bin.savefig("%s_rel_stat_uncs_%s_zoomedTo20_blue.pdf"%(process,bname))	#pdf
